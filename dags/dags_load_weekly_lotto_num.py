...
with DAG(
    ...
    def insrt_postgres(postgres_conn_id="conn-db-postgres-custom", tbl_nm="lotto_add_table", file_nm=None):
        """
        저장된 CSV 파일을 PostgreSQL 데이터베이스에 로드하는 함수입니다.
        """
        from hooks.custom_postgres_hook_lotto import CustomPostgresHookLotto
        file_path = file_nm or '/opt/airflow/files/TbLottoAdd/{{ data_interval_end.in_timezone("Asia/Seoul") | ds_nodash }}/TbLottoStatus.csv'
    ...
    def upload_to_s3(filename=None, key=None, bucket_name="morzibucket"):
        ...
        file_path = filename or '/opt/airflow/files/TbLottoAdd/{{ data_interval_end.in_timezone("Asia/Seoul") | ds_nodash }}/TbLottoStatus.csv'
        s3_key = key or 'lotto/{{ data_interval_end.in_timezone("Asia/Seoul") | ds_nodash }}/TbLottoStatus.csv'
    ...
    def generate_dag_report():
        ...
        for dag_run in dag_runs:
            ...
            if dag_run.start_date and dag_run.end_date:
                report_data[dag_id]['total_runtime'] += (dag_run.end_date - dag_run.start_date).total_seconds()
