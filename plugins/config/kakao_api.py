...
def send_kakao_msg(talk_title: str, content: dict):
    ...
    while True:
        ...
        for title, msg in content.items():
            ...
            button_lst.append(
                {"title": "", "link": {"web_url": "", "mobile_web_url": ""}}
            )
        ...
        if response.status_code == 200:  # 200: 정상
            ...
            response.status_code == 401 and try_cnt <= 2
        ):  # 401: Unauthorized (토큰 만료 등)
            _refresh_token_to_variable()
        elif (
            response.status_code != 200 and try_cnt >= 3
        ):  # 400, 401 에러가 아닐 경우 3회 시도때 종료
            return response.status_code
